...
export function VehiclesListPage() {
  ...
  <table className="table"><thead><tr><th>Plate</th><th>Make</th><th>Model</th><th>Year</th><th>Payload</th><th>Ramp</th><th/></tr></thead><tbody>
  {data.map(v=><tr key={v.id}><td>{v.registrationPlate}</td><td>{v.make}</td><td>{v.model}</td><td>{v.year}</td><td>{v.payloadCapacityKg}</td><td>{v.hasRamp?'Yes':'No'}</td><td style={{display:'flex',gap:8}}><Link className="button secondary" to={`/vehicles/${v.id}/edit`}>Edit</Link><button className="button danger" onClick={async()=>{await vehicleApi.remove(v.id); queryClient.invalidateQueries({queryKey:['vehicles']});}}>Delete</button></td></tr>)}
  </tbody></table></div>;
}
...
function VehicleForm({ mode }: { mode:'create'|'edit' }) {
  ...
  const [form, setForm] = useState({ registrationPlate:'', make:'', model:'', year: new Date().getFullYear(), payloadCapacityKg:0, hasRamp:false });
  const value = existing.data ?? form;
  const set = (k: string, v: string|number|boolean) => setForm((p)=>({ ...p,[k]: v }));
  const submit = async (e: FormEvent)=>{ e.preventDefault(); const payload = existing.data ? { ...existing.data, ...form } : form; if(mode==='create') await vehicleApi.create(payload); else await vehicleApi.update(id!, payload); nav('/vehicles'); };
  ...
  return <form className="card form-grid" onSubmit={submit}><h3>{mode==='create'?'Create':'Edit'} Vehicle</h3>
    ...
    <input type="number" value={value.payloadCapacityKg} onChange={e=>set('payloadCapacityKg',Number(e.target.value))} required />
